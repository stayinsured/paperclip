  ...
  it("returns active and terminated agents in the strict audit projection without resolving secrets or skills", async () => {
    mockAgentService.list.mockResolvedValue([{
      ...
      adapterConfig: {
        ...
        promptTemplate: "do-not-return-prompt",
        instructionsFilePath: "/do/not/return/AGENTS.md",
        env: {
          GITHUB_ACCESS: {
            type: "secret_ref",
            secretId: "33333333-3333-4333-8333-333333333333",
          },
          RAILWAY_TOKEN: { type: "plain", value: "do-not-return-env" },
        },
        paperclipSkillSync: {
      ...
      runtimeConfig: {
        ...
        modelProfiles: { cheap: { adapterConfig: { model: "do-not-return-profile" } } },
      },
    ...
    mockAccessService.decide.mockResolvedValue({
      ...
      explanation: "Allowed for an active same-company QA agent.",
    });
    ...
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl).get(`/api/companies/${companyId}/agent-metadata-audit`),
    );
    ...
    expect(mockCompanySkillService.listRuntimeSkillEntries).not.toHaveBeenCalled();
    expect(JSON.stringify(res.body)).not.toMatch(/do-not-return/);
  });

  it("denies the metadata audit route when the dedicated authorization action is denied", async () => {
    mockAccessService.decide.mockResolvedValue({
      allowed: false,
    ...
    expect(res.body.error).toContain("requires the QA agent role");
    expect(mockAgentService.list).not.toHaveBeenCalled();
  });

  it("denies cross-company metadata audits before querying the roster", async () => {
    ...
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl).get(`/api/companies/${otherCompanyId}/agent-metadata-audit`),
    );
