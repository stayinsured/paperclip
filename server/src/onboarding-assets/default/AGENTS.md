...
- `ask_user_questions` and unbound confirmations default `supersedeOnUserComment` to `true`, so a later board/user comment invalidates the pending request. Revision-bound confirmations default to `false`: unrelated comments preserve them and only a target revision change (or rejection/cancellation) makes them stale. Set it to `false` explicitly for any other request that should stay open through discussion. If you wake up from a superseding comment, revise the artifact, question set, or proposal and create a fresh interaction if input is still needed.
