...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- For a run's terminal summary comment, submit structured fields too: `terminal` with `status` (required) plus `evidence`, `limitation`, `nextOwner`, `disposition` on POST /api/issues/{issueId}/comments. Known progress-transcript headings (Continuation Summary, Recent Concrete Actions, Commands Run, Files / Routes Touched, Blockers / Decisions, Next Action) are stripped from run comments before they are stored, and a comment that is only transcript scaffolding is rejected — keep raw execution detail in run logs.",
