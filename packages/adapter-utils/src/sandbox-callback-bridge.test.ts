...
describe("sandbox callback bridge", () => {
  ...
  it("permits the documented heartbeat surface and denies unrelated routes", () => {
    ...
    const denied: Array<{ method: string; path: string }> = [
      ...
      { method: "POST", path: "/api/issues/issue-1/attachments" },
      { method: "DELETE", path: "/api/attachments/attachment-1" },
      { method: "POST", path: "/api/attachments/attachment-1/content" },
      { method: "GET", path: "/api/attachments/attachment-1/content/extra" },
